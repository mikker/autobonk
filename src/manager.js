...
export class Manager extends ReadyResource {
  ...
  async createContext (opts = {}) {
    ...
    const context = new this.ContextClass(contextStore, { schema: this.schema, bootstrap: this.bootstrap })
    await context.ready()
  ...
  async joinContext (invite, opts = {}) {
    ...
    const pairer = this.ContextClass.pair(tempStore, invite, { schema: this.schema, bootstrap: this.bootstrap })
  ...
  async listContexts () {
    ...
    const contexts = []
    for await (const { value } of this.localDb.createReadStream({ gte: 'contexts/', lt: 'contexts/~' })) {
      contexts.push(value)
    }
